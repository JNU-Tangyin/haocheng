...
# 读取数据
df = pd.read_csv('./Data/SZA/SZA.csv')  # 替换为实际的文件路径
...
df.to_csv('./Data/SZA/data_processed.csv')
...
labels.to_csv('./Data/SZA/SZA_label.csv',index=True)
...
y_pred=pd.DataFrame(y_pred,columns=['reward_pred'])
y_pred.index=y.index
y_pred=pd.concat([labels.dropna(),y_pred],axis=1)
y_pred.to_csv('./Data/SZA/SZA_pred.csv',index=True)

# 保存模型
#with open('./Data/HBEA/HBEA_model.pkl', 'wb') as file:
    #joblib.dump(model, file)
#joblib.dump(model, filename='./Data/HBEA/HBEA_model.pkl')
model.summary()
model.save('./Data/SZA/SZA_model')